...
//            return JdbcTemplateObject.queryForObject(sql, new Object[]{name}, String.class);
    ...
    public void addUser(User user) {
        ...
        try {
            JdbcTemplateObject.update(sql, user.getId(),user.getName(),user.getPassword(),user.getPhone(),user.getAvatar(),user.getEmail(),user.getTotalSpaceSize(),user.getUsedSpace(),user.getGender(),user.getAge(),user.getBirthday());
        } catch (Exception e) {
            // 处理异常
            e.printStackTrace();
        }
