...
//    }
    ...
    public List<FolderInfo> getRootFolder(String userName) {
        ...
        int userId = JdbcTemplateObject.queryForObject(sql,Integer.class,userName);
        String sql2 = "select * from folder where userId = ? and pid =0";
        List<FolderInfo> topFolders = JdbcTemplateObject.query(sql2, new BeanPropertyRowMapper<>(FolderInfo.class), userId);
        return topFolders;
        //List<FolderInfo>list=new ArrayList<>();
        //list.add(new FolderInfo(1,1,1,"folder1",0));
        //list.add(new FolderInfo(2,1,1,"folder2",0));
        //return list;
    }
